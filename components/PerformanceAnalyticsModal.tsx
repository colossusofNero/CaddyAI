...
export function PerformanceAnalyticsModal({ isOpen, onClose }: PerformanceAnalyticsModalProps) {
  const t = useTranslations('marketing.modal.analytics');
  ...
  const recentTrends = [
    { metric: t('tDriveAcc'), current: 58, trend: 'up', change: '+8%', color: 'green' },
    { metric: t('tGIR'), current: 44, trend: 'up', change: '+6%', color: 'green' },
    { metric: t('tAvgPutts'), current: 32.8, trend: 'down', change: '-2.1', color: 'green' },
    { metric: t('tAvgScore'), current: 86.4, trend: 'down', change: '-3.6', color: 'green' },
    { metric: t('tPenalty'), current: 2.1, trend: 'down', change: '-0.8', color: 'green' },
  ];

  const strengthsWeaknesses = {
    strengths: [
      { area: t('sShortGameTitle'), score: 92, description: t('sShortGameDesc') },
      { area: t('sIronPlayTitle'), score: 87, description: t('sIronPlayDesc') },
      { area: t('sCourseMgmtTitle'), score: 85, description: t('sCourseMgmtDesc') },
    ],
    weaknesses: [
      { area: t('wDriveDistTitle'), score: 58, description: t('wDriveDistDesc') },
      { area: t('wLongPuttsTitle'), score: 62, description: t('wLongPuttsDesc') },
      { area: t('wBunkerTitle'), score: 64, description: t('wBunkerDesc') },
    ],
  };

  const scoringBreakdown = [
    { category: t('catEagles'), count: 2, percent: 0.4 },
    { category: t('catBirdies'), count: 38, percent: 8.1 },
    { category: t('catPars'), count: 267, percent: 56.9 },
    { category: t('catBogeys'), count: 124, percent: 26.4 },
    { category: t('catDouble'), count: 38, percent: 8.1 },
  ];
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 bg-white border-2 border-primary rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto p-8 animate-scale-in">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5 border border-blue-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-blue-900">{t('avgScore')}</span>
              <BarChart3 className="w-5 h-5 text-blue-600" />
            </div>
            <p className="text-3xl font-bold text-blue-900">{stats.avgScore}</p>
            <p className="text-xs text-blue-700 mt-1">{t('roundsSuffix', { n: stats.roundsPlayed })}</p>
          </div>

          <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-5 border border-purple-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-purple-900">{t('handicap')}</span>
              <Award className="w-5 h-5 text-purple-600" />
            </div>
            <p className="text-3xl font-bold text-purple-900">{stats.handicap}</p>
            <p className="text-xs text-purple-700 mt-1">{t('handicapSub')}</p>
          </div>

          <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-5 border border-green-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-green-900">{t('bestScore')}</span>
              <Trophy className="w-5 h-5 text-green-600" />
            </div>
            <p className="text-3xl font-bold text-green-900">{stats.bestScore}</p>
            <p className="text-xs text-green-700 mt-1">{t('bestScoreSub')}</p>
          </div>

          <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl p-5 border border-orange-200">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-orange-900">{t('improvement')}</span>
              <TrendingUp className="w-5 h-5 text-orange-600" />
            </div>
            <p className="text-3xl font-bold text-orange-900">+{stats.improvement}%</p>
            <p className="text-xs text-orange-700 mt-1">{t('improvementSub')}</p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-4">{t('strengths')}</h3>
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-4">{t('weaknesses')}</h3>
            <div className="space-y-3">
        ...
        {/* Scoring Breakdown */}
        <div className="mb-6">
          <h3 className="text-xl font-bold text-gray-900 mb-4">{t('breakdown')}</h3>
          <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
        ...
        <div className="bg-gradient-to-r from-primary/10 to-blue-50 border border-primary/20 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="font-bold text-gray-900 mb-2">{t('insights')}</h4>
