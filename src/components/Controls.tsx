...
type Props = {
  ...
  q: {
    ...
    confidence: number;
    fairwayWidthAtDriverYds: number | null;
  };
  ...
  // live edit of club distances from active profile
  ppm?: PPM;
...
export default function Controls({
  ...
  const addHazard = () => {
    setQ((prev: any) => ({
      ...
      hazards: [
        ...(prev.hazards || []),
        { type: 'bunker', side: 'right', startYds: 250, clearYds: 265, risk: 3 } as Hazard,
      ],
    }));
  };
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className="text-sm text-gray-600 dark:text-gray-300">
            ...
            <input
              ...
              onChange={(e) =>
                setEnv((prev: any) => ({
                  ...prev,
                  windSpeed: numberOr(e.target.value, prev.windSpeed),
                }))
              }
              className="mt-1 w-full px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
            />
          ...
          <label className="text-sm text-gray-600 dark:text-gray-300">
            ...
            <input
              ...
              onChange={(e) =>
                setEnv((prev: any) => ({
                  ...prev,
                  elevationFt: numberOr(e.target.value, prev.elevationFt),
                }))
              }
              className="mt-1 w-full px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
            />
          ...
          <label className="text-sm text-gray-600 dark:text-gray-300">
            ...
            <input
              ...
              onChange={(e) =>
                updateQ({
                  fairwayWidthAtDriverYds:
                    e.target.value === '' ? null : numberOr(e.target.value, q.fairwayWidthAtDriverYds ?? 0),
                })
              }
        ...
        {hazards.length === 0 ? (
          ...
          <div className="space-y-3">
            {hazards.map((hz, i) => (
              <div
                key={i}
                className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end border border-gray-200 dark:border-gray-700 rounded-xl p-3"
              >
                ...
                <label className="text-sm text-gray-600 dark:text-gray-300 md:col-span-1">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateHazard(i, {
                        risk: Math.max(1, Math.min(5, numberOr(e.target.value, hz.risk))),
                      })
                    }
                    className="mt-1 w-full px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-800 border"
                  />
                ...
                <div className="md:col-span-1 flex md:justify-end">
                  <button
                    ...
                    className="px-3 py-2 rounded-lg bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-300 dark:border-red-900/40"
                  >
                    Remove
                  </button>
                </div>
          ...
          {showClubs && (
            <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
              {Object.entries(ppm).map(([club, spec]) => (
                <div
                  key={club}
                  className="flex items-center gap-2 border border-gray-200 dark:border-gray-700 rounded-xl p-2"
                >
